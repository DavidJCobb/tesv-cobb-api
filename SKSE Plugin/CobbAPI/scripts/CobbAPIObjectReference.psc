...
;
;  - Only usable on references whose base form ID is 0x00C0BB00. CobbPositioner.esp injects an Activator 
;    there for this purpose. Don't tamper with it, please.
;
;  - After tampering with primitive data, you need to disable and enable the reference in order to force 
;    it to reload/recompute its 3D. Primitive data "works" because the game automatically generates 3D 
;    with collision data for the reference.
;
...
Float[]         Function GetTeleportMarkerCoordinates        (ObjectReference akSubject) Global Native ; NOTE: Teleport markers are "backwards." Given linked doors A and B, Door A's teleport marker is the one that exists in front of Door B, and vice versa.
                ...
                Function ResetTeleportMarker                 (ObjectReference akSubject) Global Native ; Return a teleport marker to its original position, if it's been moved by functionality elsewhere in CobbPos.dll. We cannot undo moves by other DLLs.
...
ObjectReference[] Function GetActivateParents       (ObjectReference akSubject, Form akOptionalBaseFormFilter = None) Global Native ; if this is still commented out, then it's been omitted from the DLL because it crashes
